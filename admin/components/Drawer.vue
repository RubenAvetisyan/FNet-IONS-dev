...
<template>
  <ClientOnly>
    ...
    >
      ...
      >
        <div
          class="i-mdi-arrow-left-thick text-[#5723ae] text-md transition duration-500"
      ...
      <div class="overflow-y-auto" :class="[(isVisible && 'p-4')]">
        <ul v-if="adminLeftPanel.list.length" class="space-y-1">
          <n-list-item
            v-for="item in adminLeftPanel.list" :key="item.name" :exact="item?.type === 'link'"
            :link="item?.link || ''" :is-menuitem="item.type === 'button'"
          >
            <div
              class="hover:bg-[#5723ae] select-none items-center w-full h-12 text-base font-normal text-gray-900 hover:text-light transition duration-75 group dark:text-white dark:hover:bg-gray-700" :class="[
            ...
            <template #list>
              <n-list-item
                v-for="sub in item?.sub" :key="sub.name" :exact="sub?.type === 'link'"
