...
isActive.value = !!ready.value as boolean
if (!isActive.value) {
...
const syncButtonFn = async () => {
    const { data } = await useFetch('/api/syncmanupulation', {
        query: {
            start: isActive.value ? 0 : 1,
            stop: isActive.value ? 1 : 0
        }
    })
...
const color = computed(() => {
    return unref(isActive.value) ? 'bg-red-700 hover:bg-red-600' : 'bg-purple-700 hover:bg-purple-600'
})
...
<template>
    <admin-card>
        <template #head>{{ isActive ? 'LanBilling' : 'ABilling' }}</template>
        {{ message }}
    ...
    color,
        ...
        <template #bottom-right>
            <button
                ...
                @click="goToLogPage">
                <div class="i-mdi-table-sync h-6 w-6"></div>
            </button>
            <tooltip invisible>
