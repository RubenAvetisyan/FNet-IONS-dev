select * from (SELECT DISTINCT
    ...
    CT.title AS 'Тариф',
    if(CTO.time_to > curdate(), tariff_option.title, '') AS `discount`,
		CASE
    ...
    PARAMS.city AS 'city',
		PARAMS.area AS 'area',
		PARAMS.quarter AS 'qvartal',
		PARAMS.street AS 'street',
		CONCAT(PARAMS.house, PARAMS.frac, ' ', PARAMS.comment) AS 'house',
		if(isnull(PARAMS.flat), '', CONCAT('кв. ', PARAMS.flat)) as 'appartment',
		if(isnull(PARAMS.room) OR length(PARAMS.room) = 0, '', CONCAT('комната ', PARAMS.room)) as 'room',
		if(isnull(PARAMS.pod) OR PARAMS.pod < 1 , '', CONCAT('под. ', PARAMS.pod)) as 'entrance',
		if(isnull(PARAMS.floor) OR (PARAMS.flat >=0 AND PARAMS.floor < 1), '', CONCAT('эт. ', PARAMS.floor)) as 'floor'
FROM
    billing.contract
...
ORDER BY contract.scid DESC, contract.id DESC) AS MAIN
WHERE MAIN.`Тариф` IS NOT NULL