...
SELECT * FROM (
		...
		WHERE process.type_id IN (7, 59) AND process.status_id IN (9, 32)) AS EXCLUDE ON EXCLUDE.serviceConnection_id = process.id
		...
		WHERE process.type_id IN (7, 59) AND process.status_id in (9, 32) AND EXCLUDE.serviceConnection_id IS NULL
