...
<template>
    <li :key="storeKey" :links-key="storeKey" :data-tooltip-target="tooltipText ? tooltipId(name) : null"
      :data-tooltip-placement="tooltipPlacement" relative>
      <NTooltip v-if="tooltipText" :id="tooltipId(storeKey)">{{ tooltipText }}</NTooltip>
      ...
      <nuxt-link v-if="getRoutes().some(({ path }) => path === props.link || path === props.href) || isMenuitem"
        ...
        <div v-else>
          <slot></slot>
          <NTooltip v-if="tooltipText" :id="tooltipId(name)">{{ tooltipText }}</NTooltip>
        </div>
      </nuxt-link>
      <div v-else>
